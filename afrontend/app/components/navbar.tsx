...
export default function Navbar() {
  ...
  return (
  <nav className="fixed top-4 left-1/2 -translate-x-1/2 w-[95%] z-50 rounded-full bg-white/10 backdrop-blur-lg border border-white/20 shadow-lg transition-all duration-300">
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div className="flex justify-between h-16 items-center">
        {/* Logo */}
        <Link
          href="/"
          className="text-3xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-purple-800 via-pink-600 to-fuchsia-700 drop-shadow-md"
        >
          ARZONA
        </Link>
    ...
    <div
      className={`md:hidden fixed top-0 left-0 h-full w-64 bg-black/95 backdrop-blur-sm shadow-xl transform transition-transform duration-500 ease-in-out z-40 ${
        isOpen ? "translate-x-0" : "-translate-x-full"
      }`}
    >
      <div className="flex justify-between items-center p-4 border-b border-neutral-800">
        <span className="text-lg font-bold text-blue-500">ARZONA</span>
        <button
